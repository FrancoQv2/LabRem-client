...
const TableQueryPaginated = ({ idLaboratorio, idUsuario, tableHeaders, useHook, setComponentRef }) => {
  ...
  return (
    <>
      <TableInstance tableData={tableData} tableHeaders={tableHeaders} innerRef={innerRef} />
    </>
  )
}
...
const TableInstance = ({ tableData, tableHeaders, innerRef }) => {
  ...
  return <TableLayout {...tableInstance} innerRef={innerRef} />
}
...
const TableLayout = ({
  ...
  return (
    <>
      <Table striped responsive bordered hover size='sm' className='text-center' {...getTableProps()} ref={innerRef}>
        <thead>
      ...
      <Row className='my-2 text-center align-items-center d-flex justify-content-between'>
        <Col sm={4} lg={6}>
          <ButtonGroup size='sm'>
            ...
            <Button variant='secondary' className='mx-1' onClick={() => previousPage()} disabled={!canPreviousPage}>
              Previo
            </Button>
            <Button variant='secondary' className='mx-1' onClick={() => nextPage()} disabled={!canNextPage}>
              Siguiente
            </Button>
